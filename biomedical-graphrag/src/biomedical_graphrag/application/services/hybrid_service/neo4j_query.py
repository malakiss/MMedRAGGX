...
class Neo4jGraphQuery:
    ...
    # =====================================================
    # ============= RADIOLOGY IMAGE QUERIES ===============
    # =====================================================

    def get_similar_images_by_finding(self, finding: str, limit: int = 10) -> list[dict[str, Any]]:
        """
        Find radiology images with a specific finding.

        Args:
            finding: Radiological finding (e.g., 'pneumothorax', 'cardiomegaly')
            limit: Maximum number of images to return

        Returns:
            List of images with the specified finding
        """
        cypher = """
            MATCH (f:Finding)
            WHERE toLower(f.name) = toLower($finding)
            MATCH (img:RadiologyImage)-[:HAS_FINDING]->(f)
            RETURN img.image_id AS image_id,
                   img.image_path AS image_path,
                   img.report AS report,
                   img.modality AS modality
            LIMIT $limit
        """
        return self.query(cypher, {"finding": finding, "limit": limit})

    def get_image_report(self, image_id: str) -> list[dict[str, Any]]:
        """
        Get the full report for a specific radiology image.

        Args:
            image_id: Unique identifier for the image

        Returns:
            Image details with full report
        """
        cypher = """
        ...
        """
        return self.query(cypher, {"image_id": image_id})

    def get_images_by_modality(self, modality: str, limit: int = 20) -> list[dict[str, Any]]:
        """
        Get radiology images by imaging modality.

        Args:
            modality: Imaging modality (e.g., 'chest_xray', 'ct', 'mri')
            limit: Maximum number of images to return

        Returns:
            List of images with the specified modality
        """
        cypher = """
            MATCH (img:RadiologyImage)
            WHERE toLower(img.modality) = toLower($modality)
            OPTIONAL MATCH (img)-[:HAS_FINDING]->(f:Finding)
            RETURN img.image_id AS image_id,
                   img.image_path AS image_path,
                   img.report AS report,
            ...
            LIMIT $limit
        """
        return self.query(cypher, {"modality": modality, "limit": limit})
