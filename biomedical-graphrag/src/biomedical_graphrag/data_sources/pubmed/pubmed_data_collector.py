...
class PubMedDataCollector(BaseDataSource):
    ...
    async def fetch_citations(self, paper_id: str) -> dict:
        """
        Safe async citation fetch with timeout, retries, and fallback.
        """
        logger.debug(f"Fetching citations for PMID={paper_id}")
        await self._rate_limit()

        try:
            # Limit to 10 seconds per citation request
            citations = await asyncio.wait_for(
                self.api.fetch_citations(paper_id),
                timeout=10
            )

            # If API returned None or invalid data, fallback
            if not isinstance(citations, dict):
                raise ValueError("Invalid citations format")

            return {"pmid": paper_id, **citations}

        except asyncio.TimeoutError:
            logger.warning(f"Citation fetch timed out for PMID={paper_id}")
        except Exception as e:
            logger.warning(f"Citation fetch failed for PMID={paper_id}: {e}")
